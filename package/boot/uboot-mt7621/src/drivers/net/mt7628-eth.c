...
static int mt7628_set_link(struct udevice *dev, int port, bool enable)
{
	u32 val;
	int ret;
	...
	struct mt7628_eth_dev *priv = dev_get_priv(dev);
	ret = mii_mgr_read(priv, port, MII_BMCR, &val);
	if (ret) {
		return ret;
	}
	...
	mii_mgr_write(priv, port, MII_BMCR, val);
