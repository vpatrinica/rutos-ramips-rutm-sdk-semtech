...
static int env_sf_save(void)
{
	u32	saved_size = 0, sector;
	u32 saved_offset = CONFIG_ENV_OFFSET;
	u32	sect_size = CONFIG_ENV_SECT_SIZE;
	...
	if (sect_size > CONFIG_ENV_SIZE) {
		saved_size = sect_size - CONFIG_ENV_SIZE;
		/*saved_offset = CONFIG_ENV_OFFSET + CONFIG_ENV_SIZE; // env at the start*/
		saved_offset = CONFIG_ENV_OFFSET - saved_size; // env at the end
	...
	puts("Erasing SPI flash...");
	ret = spi_flash_erase(env_flash, saved_offset,
		sector * sect_size);
	if (ret)
		goto done;
